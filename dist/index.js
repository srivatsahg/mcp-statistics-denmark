...
import { getSubjects, getTables, searchTables, getTableInfo, getData, } from "./dst-client.js";
const TOOLS = [
    {
        name: "list_subjects",
...
const server = new Server({ name: "mcp-statistics-denmark", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "list_subjects": {
                const subjectId = args?.subject_id || undefined;
...
async function main() {
    ...
    console.error("DST MCP Server running — Statistics Denmark StatBank API ready");
}
main().catch((err) => {
